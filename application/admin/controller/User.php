<?php
// +----------------------------------------------------------------------
// | snake
// +----------------------------------------------------------------------
// | Copyright (c) 2016~2022 http://baiyf.cn All rights reserved.
// +----------------------------------------------------------------------
// | Licensed ( http://www.apache.org/licenses/LICENSE-2.0 )
// +----------------------------------------------------------------------
// | Author: NickBai <1902822973@qq.com>
// +----------------------------------------------------------------------
namespace app\admin\controller;


use app\admin\model\UserModel;
use app\admin\model\UserType;
use org\freebook\model\Dictcity AS DictcityModel ;
use think\Controller;
use think\Session;

class User extends Controller
{
    //用户列表
    public function index()
    {
        if(request()->isAjax()){

            $param = input('param.');

            $limit = $param['pageSize'];
            $offset = ($param['pageNumber'] - 1) * $limit;

            $where = [];
            if (isset($param['searchText']) && !empty($param['searchText'])) {
                $where['username'] = ['like', '%' . $param['searchText'] . '%'];
            }
            $user = new UserModel();
            $id=Session::get('id');
            if($id>1){
                $where['parent_id'] = $id;
            }
            $selectResult = $user->getUsersByWhere($where, $offset, $limit);

            $status = config('user_status');

            foreach($selectResult as $key=>$vo){

                $selectResult[$key]['last_login_time'] = date('Y-m-d H:i:s', $vo['last_login_time']);
                $selectResult[$key]['status'] = $status[$vo['status']];

                $operate = [
                    '编辑' => url('user/userEdit', ['id' => $vo['id']]),
                    '重置密码' =>"javascript:resetPassword('".$vo['id']."')",
                ];

                $selectResult[$key]['operate'] = showOperate($operate);
                
                if( 1 == $vo['id'] ){
                	$selectResult[$key]['operate'] = '';
                }
            }

            $return['total'] = $user->getAllUsers($where);  //总数据
            $return['rows'] = $selectResult;

            return json($return);
        }

        return $this->fetch();
    }

    //添加用户
    public function userAdd()
    {
        $dictcity_model=new DictcityModel();
        if(request()->isPost()){

            $param = input('param.');
            $param = parseParams($param['data']);
            if(empty($param['password'])){
                $param['password'] = md5('123456');
                //unset($param['password']);
            }else{
                $param['password'] = md5($param['password']);
            }
            $user=Session::get('id');
            $param['parent_id']=$user;
            $user = new UserModel();
            $flag = $user->insertUser($param);
            return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
        }

        $role = new UserType();
        $where=['parent_id'=>420000];
        $city_list=$dictcity_model->getAll($where);
        $this->assign('city_list',$city_list);
        $this->assign([
            'role' => $role->getRole(),
            'status' => config('user_status')
        ]);

        return $this->fetch();
    }
    /**
     * 获取区域
     * @param $cityId
     * @return bool
     */
    public function getArea(){
        $dictcity_model=new DictcityModel();
        if(request()->isPost()) {
            $city_id=input('param.city_id');
            $result=$dictcity_model->getArea($city_id);
            if($result){
                $this->ajaxMsg['code'] = 1;
                $this->ajaxMsg['msg'] = "查询成功";
                $this->ajaxMsg['resultList'] = $result;
            }else{
                $this->ajaxMsg['code'] = 2;
                $this->ajaxMsg['msg'] = "数据为空";
            }
            return json($this->ajaxMsg);
        }
    }

    //编辑角色
    public function userEdit()
    {
        $user = new UserModel();
        $dictcity_model=new DictcityModel();
        if(request()->isPost()){

            $param = input('post.');
            $param = parseParams($param['data']);
            if(empty($param['password'])){
                $param['password'] = md5('123456');
                //unset($param['password']);
            }else{
                $param['password'] = md5($param['password']);
            }
            $flag = $user->editUser($param);

            return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
        }

        $id = input('param.id');
        $role = new UserType();
        $this->assign([
            'user' => $user->getOneUser($id),
            'status' => config('user_status'),
            'role' => $role->getRole()
        ]);
        $where=['parent_id'=>420000];
        $info= $user->getOneUser($id);
        $city_list=$dictcity_model->getAll($where);
        $this->assign('city_list',$city_list);
        $area_list = $dictcity_model->getArea($info['city_id']);
        $this->assign('area_list',$area_list);

        return $this->fetch();
    }

    //删除角色
    public function userDel()
    {
        $id = input('param.id');

        $role = new UserModel();
        $flag = $role->delUser($id);
        return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
    }
    public function setting(){
        $user = new UserModel();
        $dictcity_model=new DictcityModel();
        if(request()->isPost()){

            $param = input('post.');
            $param = parseParams($param['data']);
            if(isset($param['password'])){
                $param['password'] = md5($param['password']);
            }
            $flag = $user->editUser($param);
            return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
        }
        $id=Session::get('id');
        $role = new UserType();
        $this->assign([
            'user' => $user->getOneUser($id),
            'status' => config('user_status'),
            'role' => $role->getRole()
        ]);

        $where=['parent_id'=>420000];
        $info= $user->getOneUser($id);
        $city_list=$dictcity_model->getAll($where);
        $this->assign('city_list',$city_list);
        $area_list = $dictcity_model->getArea($info['city_id']);
        $this->assign('area_list',$area_list);

        $superior=$user->getOrganicById($info['parent_id']);
        $this->assign('superior',$superior);

        return $this->fetch();
    }

    public function updatePass()
    {
        if(request()->isPost()){
            $user = new UserModel();
            $param = input('post.');
            $param = parseParams($param['data']);
            $result=$user->getpassById($param['old_password'],$param['id']);
            if(false === $result){
                return json(['code' =>2,'data'=>'','msg'=>'原密码错误']);
            }else{

                $passResult=$user->editPass($param['password'],$param['id']);
                if(false === $passResult){
                    return json(['code' =>2,'data'=>'','msg'=>'修改失败']);
                }else{
                    return json(['code' =>1,'data'=>'','msg'=>'修改成功']);
                }
            }

        }
    }
}
